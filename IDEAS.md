...
- ✅ **월드맵 배경 켄니 키트 교체 (2026-07-10)**: `media/glb/kit/`(사용 부품 16개+colormap.png, 252KB — 원본 kenney_fantasy-town-kit_2.0은 배포 제외 대상). `World.piece/kitHouse/kitSchool/kitTree`+`preloadProps`(World.go에서 자동, 실패 시 기존 박스 폴백 유지). 🔑 배색 규칙: **roof-high 세트=빨강, roof(low) 세트=초록** (Variation A 텍스처=파랑 — 미사용). 벽 패널은 자기 타일 +x 모서리에 붙음(회전으로 방향). k_hgable 삼각면은 +x(정면) 방향. 집=roof-high 게이블 3개 석조 오두막(s2.2). **학교 재설계(2026-07-10 v2)**: 집과 차별화 위해 랜드마크 회관으로 — 목재 골조(k_timberX=cross/k_timberD=diagonal) 튜더식 벽 + 아치 대문(k_doorRound+k_archTop) + 둥근창(k_winRound) + **중앙 3층 위 큰 정면 게이블**(k_hgable scale[1.9,1.15,1.0], 꼭대기 폭5.2) + 배너 + 굴뚝. 높이 10.7(집 5.4의 2배), 35부품. `piece()`에 scale 인자 추가. ⚠ 이 세션 스크린샷 도구 타임아웃(백그라운드 탭 렌더 스로틀 추정) — 구조는 좌표로 검증(에러0, 게이블 정면·배너 위치 확인)했으나 최종 시각확인은 선생님 몫. 드로우콜 40, 2396 트라이앵글.
- **남은 것**: 선생님/친구 NPC GLB(친구는 머리색·옷색 틴트로 여러명), 운동장/교실/복도 씬 키트 확장(울타리 fence.glb 등). ⚠ 안정성: examples/js CDN 실패 시 박스 폴백 동작하나, 심사 대비 three+loader 로컬 벤더링 권장. ⚠ GLB는 http 서버/https 필수 — file:// 더블클릭 시 로컬 fetch 차단되어 박스 폴백(뷰어는 `Assets.failHint()`로 안내)
